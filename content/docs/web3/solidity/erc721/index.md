...
### 6. ERC721Enumerable（可选）
这是一个让NFT支持循环遍历的接口，包括遍历所有发行的NFT、遍历用户持有的NFT。
```
interface ERC721Enumerable /* is ERC721 */ {
  // 总发行NFT数量
  function totalSupply() external view returns (uint256);

  // 查询第index个NFT的tokenId是多少
  function tokenByIndex(uint256 _index) external view returns (uint256);

  // 查询用户持有的第index个tokenId是多少
...
```
铸造NFT跟铸造ERC20代币类似，都是通过mint函数来铸造，需要注意权限控制，根据不同项目需求进行实现。  
不一样的是，NFT每个tokenId是不一样的，通常使用一个内部的自增id来记录下一个发行的tokenId，也可以使用随机数来实现（比较少见）
