...
- 线程是CPU调度的基本单位，共享进程内代码段、数据段、堆并拥有独立的栈，切换线程开销较大。  
- 协程是应用层的运行单位，在GO中通过GMP调度，切换协程开销较小。   
- 资源分配是指分配内存、IO设备、文件和网络连接、PCB等。  
- CPU调度是CPU对当前时间段哪个线程执行任务的分配。
...
不同点：  
1.线程是内核态切换开销大，协程是用户态切换开销小。  
2.线程是抢占式，一段时间会被操作系统停止并被分配给其他线程。协程是协作式，只能协程阻塞或让出时，GMP调度才会切换到其他协程。（后续go也推出了信号抢占式协程）
...
GPM是go实现的一套调度机制
- G 指 Goroutine，go协程
- P 指 Processor，逻辑处理器，负责将Goroutine任务都分派到不同的M
- M 指 Machine，操作系统线程，Go程序执行需要映射到对应的M上。  

#### 运作流程
- 一个G被创建时会进入本地队列，本地队列满了进入全局队列。  
- 当队列存在G时，调度器通过P在队列中取出G，并且P会绑定一个M，然后在M上执行G。取G时优先从本地队列中取。  
- 当G执行完成时，调度器会从队列中取出下一个G继续在绑定的M执行。  

阻塞
- 当G被阻塞时，有两种情况。第一种是通道/锁用户态阻塞，P与M不解绑，而是将G挂起，取出下一个G继续执行。
- 第二种是系统级阻塞，P与M解绑，M持续阻塞，调度器重新给P分配一个新的M，然后再取出并执行下一个G。
- 当阻塞的M释放后进入休眠，等待被一个P绑定，并把恢复的G放回队列。  
